...
export default class Main extends React.Component {
  ...
  publika () {
    ...
    let publikaCategory = {
      ...
      questions: [
        ...
        {
          text: 'Who is the President of SPIUS?',
          answers: [
            {
              text: 'Ismail Ćidić',
              correct: 1
            ...
            {
              text: 'Sid Ahmed Saidi',
              correct: 0
            }
          ]
        },
        {
          text: 'Who is director of Plavi Leptir',
          answers: [
            {
              text: 'Mehmet Yasir Cebeci',
              correct: 1
            },
            {
              text: 'Mehmet Šahin',
  ...
  hideOverlay () {
    ...
    if (this.state.quizState.overtime) {
      ...
      const divisor = quizState.round > 14 ? 2 : 14
      if (quizState.round % divisor === 0) {
      ...
      if (quizState.round % (teams * 2) === 0)
        quizState.currentCategory = null
      if (quizState.round / (teams * 2) >= this.state.categories.length) {
        this.newState = $.extend(true, {}, this.state)
        this.newState.quizState = $.extend(true, {}, quizState)
        let teams = this.getTeams()
        if (teams.length === 2) {
  ...
  render () {
    ...
    const displayRound = this.state.quizState.final ? this.state.quizState.round + 1 : this.state.quizState.round === -1 ? 0 : ((this.state.quizState.round % teamSize) + 1)
    const displayTeams = this.state.quizState.currentCategory === null ? this.state.quizState.teams : this.state.quizState.currentCategory.name === 'Trivia' ? [] : this.state.quizState.teams
    return (
      <div>
        ...
        <div className={s([style.sidebar, style.categories])}>
          ...
          <ul
            className={this.state.quizState.currentCategory === null ? style.questions : s([style.questions, style.open])}>
        ...
        <div className={
          this.state.quizState.currentQuestion === null ? style.maintop : s([style.maintop, style.active])
        }>
          <p>
            {this.state.quizState.currentQuestion !== null ? this.state.quizState.currentQuestion.text : ''
            }
          </p>
        </div>
        <div className={
          this.state.quizState.currentQuestion === null ? style.mainbottom : s([style.mainbottom, style.active])
        }>
          <ul>
            {this.state.quizState.currentQuestion !== null ? this.state.quizState.currentQuestion.answers.map(answer => {
              return (
                <li
                  key={answer.id}
                  className={s([
        ...
        <InfoBar round={displayRound}
                 ...
                 categoryName={
                   this.state.quizState.currentCategory !== null ? this.state.quizState.currentCategory.name : ''
                 }/>
          ...
          <ul>
            {displayTeams.map(team => {
              return (
                <li key={team.id}
                    className={
                      this.state.quizState.round % this.state.quizState.teams.length === teamNo++ ? style.active : ''
                    }>
        ...
        <audio id="tick" src={tick} ref={e => this.timerSound[0] = e}/>
        <audio id="tock" src={tock} ref={e => this.timerSound[1] = e}/>
      </div>
    )
  }
}